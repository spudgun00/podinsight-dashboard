...
**Testing Checkpoint:**
- Run `SELECT COUNT(*) FROM episodes` - should return exactly 0 (empty table)
...
**Testing Checkpoints:**
1. **Dry run test**: `python main.py --dry-run` - should process without errors, show "Would insert 1,171 episodes"
...
**Testing Checkpoints:**
1. **Local test**: `uvicorn api.topic_velocity:app --reload` - should start without errors on port 8000
...
**Testing Checkpoints:**
1. **Local development**: `npm run dev` - should start on http://localhost:3000
...
**Testing Checkpoints:**
1. **Deployment verification**: Visit Vercel dashboard - should show "Ready" status with green checkmark
...
#### For Database Setup:
```
I need to set up the database for PodInsightHQ. We're using Supabase.

From our playbook:
[Paste Step 1.1: Database Setup with Supabase]
...
#### For ETL Development:
```
I'm building the ETL script for PodInsightHQ. Context:
...
From our playbook:
[Paste Step 1.2: The Data Loading Script]
...
Should return data for our Topic Velocity chart (4 topics, 12 weeks)

From our playbook:
[Paste Step 2.1: FastAPI Serverless Setup]

Please create the initial project structure.
```
